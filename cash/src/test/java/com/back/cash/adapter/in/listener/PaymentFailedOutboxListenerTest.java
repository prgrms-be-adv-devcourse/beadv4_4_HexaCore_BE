...
class PaymentFailedOutboxListenerTest {

    private PaymentFailedOutboxListener listener;
    ...
    private static final String TOPIC = "cash.payment.failed";
    private static final String MOCKED_JSON = "{\"header\":{\"eventId\":\"uuid\"},\"payload\":{}}";

    @BeforeEach
    void setUp() throws Exception {
        listener = new PaymentFailedOutboxListener(outboxRepository, jsonMapper);
        setField(listener, "failedTopic", TOPIC);
    }

    @Test
    @DisplayName("record - PaymentFailedEvent → PENDING 상태로 outbox 저장")
    void record_savesOutboxAsPending() {
        // given
        PaymentFailedEvent event = new PaymentFailedEvent(RelType.ORDER, 100L, "결제 실패");
